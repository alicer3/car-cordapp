...
@InitiatingFlow
@StartableByRPC
class InsuranceAgreeFlow(val linearId: UniqueIdentifier) : FlowLogic<SignedTransaction>() {
    ...
    override fun call(): SignedTransaction {
        ...
        val txBuilder = TransactionBuilder(notary = notary)
                ...
                .addOutputState(input.copy(status = StatusEnum.AGREED), InsuranceContract.ID)
                .addCommand(command)
...
@InitiatedBy(InsuranceAgreeFlow::class)
class InsuranceAgreeFlowResponder(private val flowSession: FlowSession) : FlowLogic<SignedTransaction>() {
    @Suspendable
    override fun call(): SignedTransaction {
        val signedTransactionFlow = object : SignTransactionFlow(flowSession) {
            override fun checkTransaction(stx: SignedTransaction) {
                val output = stx.tx.outputs.first().data as Insurance
